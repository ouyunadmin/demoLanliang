  ...
  <admin-container class="grid-table">
    <div class="grid-body" flex="dir:top">
      ...
      <div flex-box="0" class="button-operation admin-mt-10">
        <el-button type="primary" plain size="mini" @click="handleCreate">新增</el-button>
        <el-button :loading="loading" :disabled="editDisabled" type="primary" plain size="mini" @click="handleUpdate">编辑</el-button>
        <el-button :loading="loading" :disabled="detailDisabled" type="primary" class="white" plain size="mini" @click="handleDetails">详情</el-button>
...
export default {
  ...
  methods: {
    ...
    gridClickFnc(res) {
      /*
       * res.field 点击的列名, 多列连接区分
       * 自定义事件
       */
      console.log(res.field)
      if (res.field === 'type') {
        Object.assign(this.detailsForm, {
          type: 'details',
          visible: true,
          data: res.id
        })
        this.$nextTick(() => {
          this.$refs.detailsDialog.init()
        })
      }
      if (res.field === 'author') {
        Object.assign(this.detailsForm, {
